...
typedef void(*fp_dev_reset)(void*);
struct lkl_fuzz_ops {
   void  (*set_dev_data)(void *handle, fp_dev_reset dev_reset, enum FUZZ_DEV_TYPE type, unsigned int irq);
   void *(*resource_alloc)(unsigned long resource_size, int resource_index);
   void  (*notify_irq_free)(unsigned int irq);
   void  (*notify_irq_request)(unsigned int irq);
   void  (*notify_devnode_add)(const char* name, unsigned long devt, int type);
   void  (*notify_devnode_remove)(const char* name, unsigned long devt, int type);
   int   (*free_irq)(unsigned int irq);
   int   (*get_free_irq)(void);
   int   (*is_active)(void);
   long  (*get_n)(void *dst, unsigned long n);
   long  (*get_rem)(void);
   void  (*delay_task)(void);
   void  (*delay_work)(void);
   void  (*delay_sirq)(int);
   int   (*minimize_delays)(void);
   int   (*minimize_wq_delays)(void);
   int   (*minimize_timeouts)(void);
   int   (*minimize_timeafter)(void);
   int   (*minimize_timebefore)(void);
   int   (*apply_hacks)(void);
   int   (*apply_patch)(void);
   int   (*apply_patch_2)(void);
   void *(*nosan_memset)(void* s, int c, unsigned long n);
   void *(*nosan_memcpy)(void *dst, const void *src, unsigned long size);
   void  (*count_syscalls)(long no, long res);
   void  (*store_dma_handle)(unsigned long long handle, unsigned long size);
   void  (*remove_dma_handle)(unsigned long long handle);
   int   (*get_dma_handle)(unsigned long long handle);
   int   (*add_waiter)(void*);
   int   (*del_waiter)(void*);
   void  (*trigger_wait_cond)(void);
   void  (*trigger_conc_cond)(void);
   void  (*scan_for_ptr_leaks)(void* addr, unsigned long size);
   void  (*is_ptr_leak)(unsigned long long addr);
   int   (*do_trace)(enum FUZZ_TRACE_TYPE type);
};
