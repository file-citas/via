...
void StackTrace::PrintBuf(char *buf, long len) const {
  int written = 0;
  if (trace == nullptr || size == 0) {
    snprintf(buf, len, "    <empty stack>\n\n");
    return;
  }
  ...
  if (len-written > 0 && dedup_token.length()) {
    snprintf(&buf[written], len-written, "DEDUP_TOKEN: %s\n", dedup_token.data());
  }
}

void StackTrace::PrintStdout() const {
  if (trace == nullptr || size == 0) {
