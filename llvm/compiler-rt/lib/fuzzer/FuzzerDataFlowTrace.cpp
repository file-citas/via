...
Vector<double> BlockCoverage::FunctionWeights(size_t NumFunctions) const {
  ...
  for (auto It : Functions) {
    ...
    if(FunctionID >= NumFunctions) {
      Printf("XXXXXXXXXXXXXXXX %s %d %d %d\n", __FUNCTION__, FunctionID, NumFunctions, Functions.size());
      continue;
    }
...
static Vector<uint8_t> ConcatTrace(Vector<uint8_t> t0, Vector<uint8_t> t1) {
  size_t len = t0.size();
  if(len < t1.size()) {
    len = t1.size();
  }
  ...
  for (auto &SF : Files) {
    ...
    while (std::getline(IF, L, '\n')) {
      ...
      //    FunctionNum == FocusFuncIdx) {
        ...
        auto NewTrace = DFTStringToVector(DFTString);
        Traces[Name] = ConcatTrace(NewTrace, Traces[Name]);
