...
static int PrintFunctions() {
  ...
  FILE *Pipe = popen("sed 's/(+/ /g; s/).*//g' "
                     ...
                     "| c++filt",
                     "w");
  for (size_t dso = 0; dso < __dft.NumDSO; dso++) {
  for (size_t I = 0; I < __dft.NumGuards[dso]; I++) {
    uintptr_t PC = __dft.PCsBeg[dso][I * 2];
    if (!BlockIsEntry(I, dso)) continue;
  ...
  //  printf("0x%lx\n", PC);
  //  void *const Buf[1] = {(void*)PC};
  //  printf("%s\n", backtrace_symbols(Buf, 1)[0]);
  //  const char *fname = FuncName((void*)PC);
  //  if(fname!=NULL)
  //    printf("%s\n", fname);
  //  else
  //    printf("NULL\n");
...
static void PrintDataFlow(FILE *Out) {
  size_t offset = 0;
  for (size_t dso = 0; dso < __dft.NumDSO; dso++) {
    for (size_t Func = 0; Func < __dft.NumFuncs[dso]; Func++) {
      bool HasAny = false;
      for (size_t Iter = 0; Iter < NumIterations; Iter++)
        if (FuncLabelsPerIter[Iter][dso][Func])
          HasAny = true;
      if (!HasAny)
        continue;
      fprintf(Out, "F%zd ", Func + offset);
      size_t LenOfLastIteration = kNumLabels;
      if (auto Tail = InputLen % kNumLabels)
        LenOfLastIteration = Tail;
      for (size_t Iter = 0; Iter < NumIterations; Iter++)
        PrintBinary(Out, FuncLabelsPerIter[Iter][dso][Func],
            Iter == NumIterations - 1 ? LenOfLastIteration : kNumLabels);
      fprintf(Out, "\n");
    }
    offset += __dft.NumFuncs[dso];
  }
}
...
static void PrintCoverage(FILE *Out) {
  ...
  for (size_t dso = 0; dso < __dft.NumDSO; dso++) {
    for (size_t FuncBeg = 0; FuncBeg < __dft.NumGuards[dso];) {
      CurrentFuncNum++;
      assert(BlockIsEntry(FuncBeg, dso));
      size_t FuncEnd = FuncBeg + 1;
      for (; FuncEnd < __dft.NumGuards[dso] && !BlockIsEntry(FuncEnd, dso); FuncEnd++)
        ;
      if (__dft.BBExecuted[dso][FuncBeg]) {
        fprintf(Out, "C%zd", CurrentFuncNum+offset);
        for (size_t I = FuncBeg + 1; I < FuncEnd; I++)
          if (__dft.BBExecuted[dso][I])
...
int main(int argc, char **argv) {
  ...
  for (size_t Iter = 0; Iter < NumIterations; Iter++) {
      FuncLabelsPerIter[Iter] =
        (dfsan_label **)calloc(__dft.NumDSO, sizeof(dfsan_label*));
    for (size_t dso = 0; dso < __dft.NumDSO; dso++) {
      FuncLabelsPerIter[Iter][dso] =
        (dfsan_label *)calloc(__dft.NumFuncs[dso], sizeof(dfsan_label));
    }
  }

  for (size_t Iter = 0; Iter < NumIterations; Iter++) {
    fprintf(stderr, "INFO: running '%s' %zd/%zd\n", Input, Iter, NumIterations);
    // we can not do that, there might still be lkl threads running
    //dfsan_flush();
