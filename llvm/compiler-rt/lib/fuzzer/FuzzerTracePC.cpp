...
ATTRIBUTES_INTERFACE_TLS_INITIAL_EXEC uint32_t __last_highwatermark;

namespace fuzzer {

TracePC TPC;

uint32_t TracePC::LastHighwaterMark() const {
  return __last_highwatermark;
}
...
void TracePC::UpdateObservedPCs() {
  ...
  auto ObservePC = [&](const PCTableEntry *TE) {
     const bool is_in = ObservedPCs.find(TE) != ObservedPCs.end();
    //if (ObservedPCs.insert(TE).second && DoPrintNewPCs) {
     if(!is_in && DoPrintNewPCs) {
      PrintPC("\tNEW_PC: %p %F %L", "\tNEW_PC: %p",
              GetNextInstructionPc(TE->PC));
      Printf("\n");
    }
    ObservedPCs.insert(TE);
  };
  ...
  auto CoveredFunctionCallback = [&](const PCTableEntry *First,
    ...
    //if (Counter) {
       for (auto PC : UncoveredPCs) {
          PrintPC("  UNCOVERED_PC: %p %F %L", "\tNEW_PC: %p",
                GetNextInstructionPc(PC));
          Printf("\n");
          //Printf("  UNCOVERED_PC: %s\n",
          //      DescribePC("%s:%l", GetNextInstructionPc(PC)).c_str());
       }
       for (auto PC : CoveredPCs) {
          PrintPC("    COVERED_PC: %p %F %L", "\tNEW_PC: %p",
                GetNextInstructionPc(PC));
          Printf("\n");
          //Printf("    COVERED_PC: %s\n",
          //      DescribePC("%s:%l", GetNextInstructionPc(PC)).c_str());
...
ATTRIBUTE_INTERFACE
ATTRIBUTE_NO_SANITIZE_ALL
void __sanitizer_prng_buf_test(uint8_t *ptr, size_t l) {
  test_ranval(ptr, l);
}


void __sanitizer_high_watermark(uint32_t mark) {
  __last_highwatermark = mark;
}

#if 0
size_t __sanitizer_max_unit_size() {
  return fuzzer::kMaxUnitSizeToPrint;
}
#endif
