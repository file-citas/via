...
// Note(feli): See comments in <lkl>/arch/lkl/fuzz/fuzz.c
// TODO(feli): Check if this matches the stream generated in lkl
// we should probably just pass the data from lkl to libfuzzer directly
#define rot(x,k) (((x)<<(k))|((x)>>(32-(k))))
u4 ranval_mod( ranctx *x ) {
  u4 e;
  e = x->a - rot(x->b, 27);
  x->a = x->b ^ rot(x->c, 17);
  x->b = x->c + x->d;
...
u4 ranval( ranctx *x , ranctx *x_mod) {
  u4 e;
  uint8_t rv_mod = (ranval_mod(x_mod)) % 256;
  if(rv_mod < prob_zero) {
    return 0;
...
void test_ranval(uint8_t *prng, size_t prng_len) {
  uint8_t val;
  u4 rv;
  size_t i, j;
  ranctx x, x_mod;
  if(prng == NULL || prng_len == 0) {
    return;
  }
  raninit(&x, 0x42);
  raninit(&x_mod, 0x23);
  for(i=0; i<prng_len; i+=sizeof(u4)) {
    rv = ranval(&x, &x_mod);
    for(j=0; j<sizeof(u4); j++) {
      if(i+j < prng_len) {
        val = (rv & (0xffUL << (j*8))) >> (j*8);
        if(prng!=NULL && prng_len >= i+j) {
          if(val != prng[i+j]) {
            fprintf(stderr, "ERROR PRNG MISMATCH %d %x != %x (%lx)\n", i+j, val, prng[i+j], rv);
            //fprintf(stdout, "ERROR PRNG MISMATCH %d %x != %x (%lx)\n", i+j, val, prng[i+j], rv);
          }
        }
      }
    }
...
void memset_ranval(uint8_t *dst, size_t n) {
  u4 rv;
  size_t i, j;
  ranctx x, x_mod;
  raninit(&x, 0x42);
  raninit(&x_mod, 0x23);
  for(i=0; i<n; i+=sizeof(u4)) {
    rv = ranval(&x, &x_mod);
    for(j=0; j<sizeof(u4); j++) {
      if(i+j < n) {
        dst[i+j] = (rv & (0xff << (j*8))) >> (j*8);
      }
    }
  }
}
...
void Fuzzer::PrintStats(const char *Where, const char *End, size_t Units,
  ...
  if (!Options.Verbosity)
    return;
  Printf("%d #%zd\t%s", time(NULL), TotalNumberOfRuns, Where);
...
bool Fuzzer::RunOne(uint8_t *Data, size_t Size, bool MayDeleteFile,
                    InputInfo *II, bool *FoundUniqFeatures, size_t *NewSize) {
  if(NewSize != NULL) {
     *NewSize = Size;
  }
  if (!Size)
  ...
  if(NewSize != NULL) {
      if(TPC.LastHighwaterMark() > 0 && TPC.LastHighwaterMark() < Size) {
         //fprintf(stderr, "Reducing data size %ld -> %ld\n", Size, TPC.LastHighwaterMark());
         Size = TPC.LastHighwaterMark();
         *NewSize = Size;
      } else if (TPC.LastHighwaterMark() > Size && TPC.LastHighwaterMark() < MaxInputLen) {
         //fprintf(stderr, "Increasing data size %ld -> %ld\n", Size, TPC.LastHighwaterMark());
         memset_ranval(Data+Size, TPC.LastHighwaterMark()-Size);
         Size = TPC.LastHighwaterMark();
         *NewSize = Size;
      } else if (TPC.LastHighwaterMark() >= MaxInputLen) {
         //fprintf(stderr, "Warning Increasing data size would cause overflow %ld -> %ld (max %ld)\n", Size, TPC.LastHighwaterMark(), MaxInputLen);
         if(Size < MaxInputLen-1) {
            memset_ranval(Data+Size, MaxInputLen-1-Size);
         }
         Size = MaxInputLen -1;
         *NewSize = Size;
      }
  ...
  if (NumNewFeatures) {
    ...
    InputInfo *NewII;
    //if (TPC.LastHighwaterMark() > Size) {
    //  fprintf(stderr, "Increasing data size %ld -> %ld\n", Size, TPC.LastHighwaterMark());
    //  //uint8_t *NewData = new uint8_t[TPC.LastHighwaterMark()];
    //  //memcpy(NewData, Data, Size);
    //  memset_ranval(NewData+Size, TPC.LastHighwaterMark()-Size);
    //  NewII = Corpus.AddToCorpus({NewData, NewData + TPC.LastHighwaterMark()}, NumNewFeatures,
    //                                MayDeleteFile, TPC.ObservedFocusFunction(),
    //                                UniqFeatureSetTmp, DFT, II);
    //  Size = TPC.LastHighwaterMark();
    //  delete NewData;
    //} else {
    NewII = Corpus.AddToCorpus({Data, Data + Size}, NumNewFeatures,
          MayDeleteFile, TPC.ObservedFocusFunction(),
          UniqFeatureSetTmp, DFT, II);
    //}
...
void Fuzzer::WriteUnitToFileWithPrefix(const Unit &U, const char *Prefix) {
  ...
  WriteToFile(U, Path);
  //WriteToFile((uint8_t*)TPC.DmaAccessList(), TPC.DmaAccessListLen(), Path+".dma");
...
void Fuzzer::MutateAndTestOne() {
  ...
  for (int i = 0; i < Options.MutateDepth; i++) {
    ...
    bool NewCov = RunOne(CurrentUnitData, Size, /*MayDeleteFile=*/true, &II,
                         &FoundUniqFeatures, &NewSizeHWM);
    if(Size != NewSizeHWM) {
       //fprintf(stderr, "Changed data size %ld -> %ld\n", Size, NewSizeHWM);
       Size = NewSizeHWM;
    }
    //if (TPC.LastHighwaterMark() > TmpMaxMutationLen) {
    //  TmpMaxMutationLen = TPC.LastHighwaterMark();
    //}
    //if (TPC.LastHighwaterMark() > Size) {
    //  assert(TPC.LastHighwaterMark() <= MaxInputLen && "Oversized Unit");
    //  //fprintf(stderr, "Increasing current data size %ld -> %ld\n", Size, TPC.LastHighwaterMark());
    //  //memset(CurrentUnitData+Size, 0, TPC.LastHighwaterMark()-Size);
    //  memset_ranval(CurrentUnitData+Size, TPC.LastHighwaterMark()-Size);
    //  Size = TPC.LastHighwaterMark();
    //}
