...
extern "C" void __attribute__((weak))maybe_inj_dma(uptr addr, uptr size, int is_write);
extern "C" void __attribute__((weak))kasan_report(unsigned long addr, unsigned long size, bool is_write, unsigned long ip);
void ReportGenericError(uptr pc, uptr bp, uptr sp, uptr addr, bool is_write,
  ...
  if (AddrIsInMem(addr)) {
    ...
    if(shadow_val == kAsanUserPoisonedMemoryMagic) {
      maybe_inj_dma(addr, access_size, is_write);
      return;
    }
  }
  ...
  kasan_report(addr, access_size, is_write, pc);
