...
int __asan_get_allocation_context(void) {
  GET_STACK_TRACE_MALLOC;
  return StackDepotPut(stack);
}
extern "C" SANITIZER_INTERFACE_ATTRIBUTE
void __asan_print_allocation_context(int id) {
  auto stack = GetStackTraceFromId(id);
  stack.PrintStdout();
}
extern "C" SANITIZER_INTERFACE_ATTRIBUTE
void __asan_print_allocation_context_err(int id) {
  auto stack = GetStackTraceFromId(id);
  stack.Print();
}
extern "C" SANITIZER_INTERFACE_ATTRIBUTE
void __asan_print_allocation_context_buf(int id, char *buf, long len) {
  auto stack = GetStackTraceFromId(id);
  stack.PrintBuf(buf, len);
}
